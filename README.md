...
```
cackalacky-ai-village-2026/
│
├── README.md                     ← You are here
├── pyproject.toml                ← Python dependencies (managed by uv)
├── uv.lock                      ← Lockfile (deterministic installs)
├── verify.py                    ← Pre-work verification script
│
├── level1/                       ← 🟢 BEGINNER: The Solo Agent
│   ├── agent.py                  ← Starter code — fill in the blanks
│   └── challenges/
│       ├── challenge1/
│       │   └── hint.txt          ← Password puzzle (Base64)
│       ├── challenge2/
│       │   └── metadata.txt      ← OSINT forensics
│       ├── challenge3/
│       │   └── auth.log          ← Log analysis
│       ├── bonus/
│       │   └── crypto_note.txt   ← ROT13 bonus challenge
│       └── competition/
│           └── emergency.txt     ← 🏆 Live competition challenge (sealed)
│
├── level2/                       ← 🟡 INTERMEDIATE: The Recon Agent
│   ├── agent.py                  ← Starter code with new tools
│   └── challenges/
│       ├── briefing.txt          ← Mission briefing
│       ├── cve_database.json     ← Local CVE lookup data
│       └── recon/
│           ├── scan_results.xml  ← Pre-captured nmap output
│           ├── http_headers.txt  ← HTTP response headers
│           ├── dns_records.txt   ← DNS enumeration
│           ├── robots_txt.txt    ← robots.txt contents
│           └── ssl_info.txt      ← SSL/TLS certificate info
│
├── level3/                       ← 🔴 ADVANCED: The SOC Squad
│   ├── soc_squad.py              ← Multi-agent starter code
│   └── challenges/
│       ├── threat_intel_db.json  ← Local threat intelligence database
│       └── incident/
│           ├── siem_alerts.json  ← SIEM alert data
│           ├── firewall_logs.txt ← Firewall connection logs
│           └── endpoint_logs.txt ← EDR/endpoint telemetry
│
└── solutions/                    ← 🔒 Posted after the workshop
    ├── level1/
    │   └── solution.py
    ├── level2/
    │   └── solution.py
